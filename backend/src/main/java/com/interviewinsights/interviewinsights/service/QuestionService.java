...
import com.interviewinsights.interviewinsights.dto.QuestionFrequencyResponse;
...
public class QuestionService {
    ...
    public QuestionService(QuestionRepository questionRepository,EmbeddingService embeddingService,ExperienceQuestionRepository experienceQuestionRepository) {
    ...
    public List<QuestionFrequencyResponse> getQuestionFrequencyByCompany(
        Long companyId,
        QuestionCategory category,
        Integer years) {

    Integer cutoffYear = null;

    if (years != null) {
        int currentYear = java.time.Year.now().getValue();
        cutoffYear = currentYear - years;
    }

    List<Object[]> results =
            experienceQuestionRepository.findQuestionFrequencyByCompany(
                    companyId,
                    category,
                    cutoffYear,
                    years
    ...
    return results.stream()
            .map(row -> new QuestionFrequencyResponse(
                    (Long) row[0],
                    (String) row[1],
